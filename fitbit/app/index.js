...
const COMPANION_WAKE_INTERVAL = 20000 // ms between attempts to wake companion
...
function sendFile(fileName) {
  ...
  function wakeCompanion() {
    ...
    errorEl.text = "Companion isn’t responding"
    ...
    //launchApp("000013f2-0000-4000-8000-000000f17b17")// If companion doesn't respond, transfer to a relanching app, restart and resume. Pass UUID of this app, and fileName to resume.
  }
}
...
function receiveFilesFromCompanion() {
  ...
  while (fileName = inbox.nextFile()) {
    ...
    if (response.fileName) {
      ...
      statusEl.text = 'Done: files are on phone'
      display.poke()
      recBtnEl.text = 'START RECORDING'
      recBtnEl.style.display = 'inline'
      xferBtnEl.style.display = 'none' // xferBtnEl.text = ''
...
function sendNextFile() {
  ...
  if (++fileNumberSending > state.fileNumberRecording) {
    ...
    statusEl.text = 'All files sent; wait...'
    display.poke()
